...
async def prepare_workout(workout_id: int) -> dict[str, Any]:
    ...
    Fetches the three v2 analysis endpoints (summary/charts/laps) and merges
    them back into the same raw-data shape the 'compute' phase already
    expects from the old v1 endpoint (see module docstring).
    ...
    async with TPClient() as client:
        ...
        async with httpx.AsyncClient(timeout=ANALYSIS_TIMEOUT) as http_client:
            summary, err = await _post_analysis(
                http_client, _SUMMARY_PATH, headers, workout_id
            )
            if err:
                return err

            charts, charts_err = await _post_analysis(
                http_client, _CHARTS_PATH, headers, workout_id
            )
            if charts_err:
                if charts_err.get("error_code") == "NOT_FOUND":
                    logger.info(
                        "workout %s: no chart/stream data available", workout_id
                    )
                    charts = None
                else:
                    return charts_err

            laps, laps_err = await _post_analysis(
                http_client, _LAPS_PATH, headers, workout_id
            )
            if laps_err:
                if laps_err.get("error_code") == "NOT_FOUND":
                    logger.info("workout %s: no lap data available", workout_id)
                    laps = None
                else:
                    return laps_err
    ...
    totals = [
        {
            "name": meta.get("friendlyName") or name,
